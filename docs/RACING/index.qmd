...
- **효과성**: primary end point occur하는 환자가 적을수록, LDL<70mg을 만족하는 환자가 많을수록 **효과성이 높다**고 판단. <br><br>
...
- **Stratified by baseline LDL cholesterol and diabetes**ㄷ
...
- 이 연구의 핵심 outcome.
- 3년 동안 **심혈관 사망, 주요 심혈관 사건, 비치명적 뇌졸증** 하나라도 발생하면, primary endpoint가 발생한 것으로 판단.

### Secondary Endpoint
- **Secondary Efficacy Endpoints**:
- 치료 효과를 추가로 확인하기 위한 outcome. 
- 1년, 2년, 3년에 LDL cholesterol <7 mg/dL 달성 여부
- primary endpoint의 개별 구성요소의 발생률
- LDL cholesterol <55 mg/dL 달성률은 post-hoc analysis로 평가함.<br><br>

- **Safety Endpoints**:
- 치료가 얼마나 안전하고 지속 가능한지를 본 outcome.
  - 약제 불내성으로 인한 약물 중단 또는 감량을 확인. 
  - 새로 발생한 당뇨/근육 관련 이상반응/ 간 관련 이상반응을 관찰
:::
...
-   Missing outcome data는 imputation 없이 censoring 또는 available data 기준으로 처리함.
-   Prespecified subgroup analysis로 결과의 일관성을 확인함.
:::


## Primary Objective and Results {#primary-objective-main}

-   Primary endpoint에 대해 **non-inferiority**를 검정함.
-   병합요법이 고강도 statin 단독요법보다 3년 primary endpoint에서 임상적으로 뒤떨어지지 않는지 평가함.
-   Noninferiority는 두 군 primary endpoint 누적발생률 차이의 confidence interval 상한이 **2.0%p 미만**일 때 충족됨.
-   이 분석은 각 환자에서 사건이 언제 발생했는지까지 반영한 [**Time-to event outcome analysis**](#time-to-event-outcome-analysis)로 분석함.
- Results: 
  -병합요법: 172/1894명, 9.1%
...
## Time-to event outcome analysis {#time-to-event-outcome-analysis}
...
- Cox Regresssion: 두 군의 event 발생 속도를 hazard ratio, HR로 비교. HR이 1이면 비슷하고, 1보다 작으면 치료군의 event 위험이 낮은 방향.<br><br>
- Results:  RACING에서는 병합요법과 고강도 스타틴 단독요법의 K-M curve가 비슷했고, Cox HR은 0.92로 1에 가까웠음. <br>
...
- 연구 질문: 병합요법이 임상적으로 고강도 statin 단독요법보다 나쁘지 않다면, LDL 조절 목표 달성 면에서는 오히려 더 우수한가? 
...
-   추가로 post-hoc 분석에서 더 엄격한 목표인 LDL-C <55 mg/dL도 병합요법군에서 더 많이 달성했습니다: 1년 42% vs 25%, 2년 45% vs 29%, 3년 42% vs 25%.

## Additional Results
:::  {.panel-tabset}
### primary endpoint 개별 구성요소
    - Cardiovascular death: 0.4% vs 0.3%
    - Major cardiovascular events: 8.1% vs 8.9%
    - Non-fatal stroke: 0.8% vs 0.7%
    - All-cause death: 1.4% vs 1.2%
    -개별 구성요소는 두 군 간 유의한 차이가 없었음. 

### Safety Outcomes
-약제 불내성으로 인한 중단 또는 감량: 4.8% vs 8.2%
...
### Subgroup analysis
- Age, sex, BMI, diabetes, hypertension, CKD, previous MI/stroke/PAD, baseline LDL-C에 따른 subgroup에서 병합요법이 명확하게 불리한 결과는 관찰되지 않았음.
...
- 예상 event rate: 13% vs 14%
  ...
  - 계산: 1605 / (1 - 0.15)
  # 1888.2
  - 따라서 1605명/군 → 탈락률 반영 후 약 1890명/군, 최종 총 3780명
...
- 대략 93명/군이 필요하고, 15% 탈락률을 반영하면:
  93 / 0.85
  # 109.41
- 따라서 110명/군, 총 220명이면 secondary objective 검정은 충분.
